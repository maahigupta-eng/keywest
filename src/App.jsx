...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .holiday-chip{display:none;align-items:center;gap:5px;margin-top:auto;align-self:flex-start;font-size:9px;letter-spacing:0.7px;text-transform:uppercase;color:#A65A23;background:rgba(232,137,74,0.12);border:1px solid rgba(232,137,74,0.22);padding:3px 7px;border-radius:20px;max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  .holiday-dot{display:none;position:absolute;right:4px;bottom:4px;width:7px;height:7px;border-radius:50%;background:var(--sunset);box-shadow:0 0 0 3px rgba(232,137,74,0.13);}
  .holiday-dot.social{background:var(--teal-light);box-shadow:0 0 0 3px rgba(91,191,163,0.14);}
  .holiday-mini{display:none;font-size:8px;color:rgba(14,26,22,0.45);position:absolute;bottom:3px;left:50%;transform:translateX(-50%);}
  ...
  /* ── JAKE CLEANUP: FAMILY-FIRST, NOT BOOKING-FIRST ── */
  .coming-strip{margin:0 0 18px;background:rgba(253,250,246,0.74);border:1px solid rgba(232,220,200,0.9);border-radius:22px;padding:16px 18px;box-shadow:0 14px 42px rgba(14,26,22,0.055);backdrop-filter:blur(10px);}
  .coming-label{font-size:10px;letter-spacing:2.4px;text-transform:uppercase;color:var(--teal-mid);font-weight:400;margin-bottom:10px;}
  .coming-list{display:flex;gap:10px;flex-wrap:wrap;align-items:center;}
  .coming-chip{border:1px solid rgba(232,220,200,0.95);background:linear-gradient(180deg,rgba(253,250,246,0.95),rgba(245,239,227,0.72));border-radius:999px;padding:9px 14px 9px 11px;display:flex;align-items:center;gap:9px;cursor:pointer;font-family:'Jost',sans-serif;box-shadow:0 8px 20px rgba(14,26,22,0.04);}
  .coming-chip::before{content:'';width:10px;height:10px;border-radius:50%;background:var(--member-color);box-shadow:0 0 0 4px color-mix(in srgb, var(--member-color) 18%, transparent);}
  .coming-chip span{font-size:13px;color:var(--ink);font-weight:400;}
  .coming-chip em{font-size:12px;color:var(--mid);font-style:normal;font-weight:300;}
  .coming-empty{font-size:13px;color:var(--light);font-weight:300;padding:4px 0;}
      ...
      <div className="auth-wrap">
        <div className="auth-left">
          ...
          <div className="auth-left-content">
            ...
            <div className="auth-left-quote">"Check dates, see who's home, and keep it easy."</div>
          </div>
        </div>
        <div className="auth-right">
          <div className="auth-title">Casa Kallman</div>
          <div className="auth-sub">{mode==="login"?"Enter to see the family calendar.":"First time here? Make a simple name/password with the house passkey."}</div>
          <div className="field-group"><label className="field-label">Name</label><input className="field-input" value={name} onChange={e=>setName(e.target.value)} placeholder="Your name" onKeyDown={e=>e.key==="Enter"&&handle()}/></div>
          ...
          <button className="btn-primary" onClick={handle} disabled={loading}>{loading?"...":(mode==="login"?"Enter Calendar":"Join Calendar")}</button>
          <div className="toggle-mode">
            {mode==="login"?<><span>First time? </span><button onClick={()=>{setMode("register");setErr("");}}>Join with house passkey</button></>:<><span>Already joined? </span><button onClick={()=>{setMode("login");setErr("");}}>Enter instead</button></>}
          </div>
        </div>
      </div>
    </div>
...
function BookingModal({booking,user,onClose,onSave,onDelete}){
  ...
  const [color,setColor]=useState(booking?.color||user?.color||DEFAULT_COLOR);
  const [loading,setLoading]=useState(false);
  ...
  const save=async()=>{
    ...
    setLoading(true);
    try{await onSave({name,startDate,endDate,note,visibility:"family",color,id:booking?.id});onClose();}
    finally{setLoading(false);}
  };
  ...
  return(
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        <div className="modal-title">{isEdit?"Edit Time at the House":"Add Time at the House"}</div>
        <div className="field-group"><label className="field-label">Who</label><input className="field-input" value={name} onChange={e=>setName(e.target.value)} placeholder="Jake, Sam, Mom + Dad..."/></div>
        ...
        <div className="field-group"><label className="field-label">Note</label><input className="field-input" value={note} onChange={e=>setNote(e.target.value)} placeholder="Optional: arriving late, with friends, family weekend..."/></div>
        <div className="field-group"><label className="field-label">Color</label><div className="color-picker">{MEMBER_COLORS.map(c=><div key={c.id} className={`color-swatch ${color===c.hex?"selected":""}`} style={{background:c.hex}} onClick={()=>setColor(c.hex)} title={c.label}/>)}</div></div>
        <div className="modal-actions">
          ...
          <button className="btn-save" onClick={save} disabled={loading}>{loading?"Saving...":"Save Dates"}</button>
        </div>
      </div>
    </div>
  );
...
function CalSidebar({bookings,isGuest}){
  ...
  return(
    <div className="cal-sidebar">
      <WeatherCard/>
      <div className="sidebar-title">Coming Up</div>
      <div className="sidebar-sub">{next?`${next.name} · ${fmt(next.startDate)} — ${fmt(next.endDate)}`:"No plans added yet"}</div>
      {upcoming.length===0?(
        <div className="sidebar-empty">
          <div className="sidebar-palm">🌴</div>
          <div className="sidebar-empty-text">Nothing added yet.<br/>Add the first stay when someone knows their dates.</div>
        </div>
      ):(
        upcoming.map(b=>{
...
function MonthView({bookings,user,isGuest,onSave,onDelete,showToast}){
  ...
  const nextStay=visible.filter(b=>b.endDate>=ts).sort((a,b)=>a.startDate.localeCompare(b.startDate))[0];
  const currentStay=visible.find(b=>b.startDate<=ts&&b.endDate>=ts);
  const upcoming=visible.filter(b=>b.endDate>=ts).sort((a,b)=>a.startDate.localeCompare(b.startDate)).slice(0,3);
  ...
  return(
    <>
      ...
      <div className="calendar-hero-row">
        <div className="house-status-card">
          <div className="house-status-label">At the House</div>
          <div className="house-status-main"><span className={`status-dot-live ${currentStay?"":"open"}`}/>{currentStay?`${currentStay.name} is here`:"Coming Up"}</div>
          <div className="house-status-sub">{currentStay?`${fmt(currentStay.startDate)} — ${fmt(currentStay.endDate)}`:nextStay?`${nextStay.name} · ${fmt(nextStay.startDate)} — ${fmt(nextStay.endDate)}`:"No dates added yet."}</div>
        </div>
        <CompactWeatherCard/>
      </div>
      <div className="coming-strip">
        <div className="coming-label">Coming up</div>
        <div className="coming-list">
          {upcoming.length?upcoming.map(b=><button key={b.id} className="coming-chip" style={{"--member-color":b.color||DEFAULT_COLOR}} onClick={()=>setDetailB(b)}><span>{b.name}</span><em>{fmt(b.startDate)} — {fmt(b.endDate)}</em></button>):<div className="coming-empty">No dates added yet.</div>}
        </div>
      </div>
      <div className="forecast-note">Daily Key West weather appears where available.</div>
      ...
      {detailB&&(
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setDetailB(null)}>
          <div className="modal">
            ...
            <div className="modal-actions">
              {user&&<button className="btn-remove" onClick={async()=>{if(window.confirm("Remove?")){await onDelete(detailB.id);setDetailB(null);showToast("Removed.");}}}>Remove</button>}
              <button className="btn-cancel" onClick={()=>setDetailB(null)}>Close</button>
              {user&&<button className="btn-save" onClick={()=>{setEditB(detailB);setDetailB(null);}}>Edit</button>}
            </div>
          </div>
        </div>
      )}
...
function WhosThereView({bookings,isGuest}){
  ...
  return(
    <div className="whos-wrap">
      ...
      <div className="section-sub">Who's coming up at Casa Kallman.</div>
      {upcoming.length===0?(
        <div className="empty-state"><div className="empty-icon">🌴</div><div className="empty-title">No dates yet</div><div className="empty-sub">Add a stay when someone knows they're coming.</div></div>
      ):(
...
function FamilyProfilesView({bookings,isGuest}){
  ...
  return(
    <div className="family-wrap">
      <div className="section-title">People</div>
      <div className="section-sub">Profiles appear automatically from names added to the calendar.</div>
      {members.length===0?(
          ...
          {members.map(m=>{
            ...
            return(
              <div key={m.name} className="family-card" style={{"--member-color":m.color}}>
                ...
                <div className="family-next">{next?<>Next time here: <strong>{fmt(next.startDate)} — {fmt(next.endDate)}</strong>{next.note?` · ${next.note}`:""}</>:"No dates added yet."}</div>
                <div className="family-note">Color can be chosen when adding dates.</div>
              </div>
            );
          })}
        </div>
  ...
  return(
    <div className="photos-wrap">
      <div className="memory-hero">
        ...
        <div className="section-sub">A quiet place for house photos. Uploads preview here for now; shared storage can be added later.</div>
...
function FamilyApp({user,bookings,onSave,onDelete,showToast,isGuest,onGoHome}){
  ...
  const tabs=[{id:"month",label:"Calendar"},{id:"family",label:"People"},{id:"photos",label:"Memories"}];
    ...
    <div className="app-shell">
      <div className="app-header">
        <div className="app-brand">
          ...
          <span className="app-brand-badge">Sunset Key</span>
        </div>
        <div className="app-header-right">
          ...
          <button className="btn-signout" onClick={signOut}>Exit</button>
        </div>
      </div>
...
export default function App(){
  ...
  return(
    <>
      <style>{css}</style>
      {screen==="auth"&&<AuthScreen onBack={()=>{}} onSuccess={(u)=>{setUser(u);setScreen("app");}}/>}
      {screen==="app"&&<FamilyApp user={user} bookings={bookings} isGuest={isGuest} onSave={handleSave} onDelete={handleDelete} showToast={showToast} onGoHome={()=>{}}/>}
      <Toast msg={toast}/>
    </>
  );
}
