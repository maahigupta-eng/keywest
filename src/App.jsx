...
const COLORS = [
  { id:"teal",    hex:"#2E9B7F" },{ id:"sunset",  hex:"#E8894A" },
...
const HOUSE_PHOTOS = [
  { src: pool_exterior, label: "The Pool" },
  { src: underwater,   label: "Pool View" },
  { src: veranda,      label: "The Veranda" },
  { src: sunset_pool,  label: "Sunset" },
  { src: kitchen,      label: "Kitchen" },
  { src: living,       label: "Living Room" },
  { src: living2,      label: "Great Room" },
];
...
const css = `
  ...
  :root {
    --ink:#0E1A16; --teal:#1A6B5A; --teal-mid:#2E9B7F; --teal-light:#5BBFA3;
    --sand:#F5EFE3; --sand-mid:#E8DCC8; --sand-dark:#C8B898;
    --sunset:#E8894A; --white:#FDFAF6; --mid:#5A7068; --light:#96B0A8;
    --seafoam:#EEF8F5;
  }
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .person-card{
    ...
    overflow:hidden;border:1px solid var(--sand-mid);
  ...
  .person-note{font-size:13px;color:var(--mid);font-style:italic;background:var(--sand);border-radius:8px;padding:10px 12px;margin-bottom:12px;font-weight:300;font-family:'Cormorant Garamond',serif;font-size:15px;line-height:1.5;cursor:pointer;transition:background 0.2s;}
  .person-note:hover{background:var(--sand-mid);}
  .person-note-input{width:100%;font-size:14px;font-style:italic;color:var(--mid);background:var(--sand);border:none;border-radius:8px;padding:10px 12px;margin-bottom:12px;font-weight:300;font-family:'Cormorant Garamond',serif;font-size:15px;outline:1.5px solid var(--teal-mid);resize:none;}
  ...
  /* ── PHOTOS PAGE ── */
  .photos-page{padding:40px;max-width:1100px;margin:0 auto;width:100%;}
  .photos-house-grid{
    display:grid;
  ...
  .photos-upload-section{margin-top:8px;}
  ...
  .upload-zone{
    border:2px dashed var(--sand-dark);border-radius:14px;
  ...
  .memory-label{position:absolute;bottom:10px;left:12px;font-size:12px;color:white;font-weight:300;font-family:'Jost',sans-serif;}
  ...
  .detail-note{background:var(--seafoam);padding:12px 16px;border-radius:10px;font-size:14px;color:var(--mid);font-style:italic;font-weight:300;margin-top:14px;}
  ...
  @media(max-width:800px){
    ...
    .cal-main,.people-page,.photos-page{padding:16px;}
...
function WeatherCard() {
  ...
  if (!wx) return null;
  const icon = WX[wx.weathercode] || "🌤️";
  const vibe = VIBES[wx.weathercode] || "Beautiful Key West day.";
  return (
      ...
      <div className="modal">
        ...
        <div className="field-group">
          ...
          <div className="color-row">
            {COLORS.map(c=><div key={c.id} className={`color-dot ${color===c.hex?"sel":""}`} style={{background:c.hex}} onClick={()=>setColor(c.hex)}/>)}
          </div>
        </div>
...
function PeoplePage({ stays }) {
  ...
  const saveNote = (name,val) => {
    const n={...notes,[name]:val};
    setNotes(n);
    localStorage.setItem("ck_notes",JSON.stringify(n));
    setEditingNote(null);
  };

  useEffect(()=>{
    try{const n=JSON.parse(localStorage.getItem("ck_notes")||"{}");setNotes(n);}catch{}
  },[]);
...
export default function App() {
  ...
  const fetchStays = useCallback(async()=>{
    try {
      ...
      setStays((d.bookings||[]).map(b=>({...b,start:b.start||b.startDate,end:b.end||b.endDate})));
    } catch {}
  },[]);

  useEffect(()=>{
    if (sessionStorage.getItem("casa_ok")==="true") setScreen("app");
  },[]);
